...
class Router implements BindingRegistrar, RegistrarContract
{
    ...
    /**
     * Routa de para redirecconar a los controladores y a sus funciones
    */
    public function controllers(array $controllers){
        foreach ($controllers as $uri => $controller)
        {
            if (isset($_SERVER["REQUEST_URI"]) && strpos($_SERVER["REQUEST_URI"],$uri)) {
                $url1 = explode($uri,$_SERVER["REQUEST_URI"]);
                $url2 = explode('?',$url1[1]);
                $urlFinal = $uri.$url2[0];
                $action = ucfirst(substr($url2[0],1));
                if ($_SERVER["REQUEST_METHOD"] == 'GET') {
                return $this->get($urlFinal,$controller.'@get'.$action);
                }else {
                return $this->post($urlFinal,$controller.'@post'.$action);
                }
            }
        }
    }
